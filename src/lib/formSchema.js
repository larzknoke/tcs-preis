...
export function createFormSchema({ isSonderpreis = false } = {}) {
  return yup.object().shape(
    {
      ...
      plzTraeger: yup
        ...
        .length(5, "Bitte eine gültige PLZ eingeben.")
        .required(),
      ...
      benachteiligungProjekt: isSonderpreis
        ? yup.string().max(500)
        : yup.string().max(500).required(),
      umsetzungProjekt: yup.string().max(500).required(),
      bisherigeErgebnisse: yup.string().max(500).required(),
      nachhaltigkeitProjekt: isSonderpreis
        ? yup.string().max(500).required()
        : yup.string().max(500),
      uebertragbarkeitProjekt: isSonderpreis
        ? yup.string().max(500).required()
        : yup.string().max(500),
