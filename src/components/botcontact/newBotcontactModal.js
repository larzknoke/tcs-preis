...
import { botcontactSchema } from "@/lib/formSchema";
import { useRouter } from "next/router";

function NewBotcontactModal({ botschafter, isOpen, onClose }) {
  ...
  const {
    ...
    resolver: yupResolver(botcontactSchema),
  });
  ...
  async function onSubmit(values) {
    try {
      values.botschafterId = botschafter.id;
      const res = await fetch("/api/botcontact", {
        method: "POST",
  ...
  return (
    <>
      <Modal isOpen={isOpen} onClose={onClose} size={"xl"}>
        <ModalOverlay />
        <ModalContent>
          <ModalHeader>Neuer Ansprechpartner</ModalHeader>
          <ModalCloseButton />
          <ModalBody>
            <form id="new-botcontact-form" onSubmit={handleSubmit(onSubmit)}>
...
export default NewBotcontactModal;
