...
function EmailBotschafterModal({ isOpen, onClose }) {
      ...
      Papa.parse(fileInput.current?.files[0], {
        ...
        complete: async function (results) {
          ...
          if (parsedData.length > 0) {
            const res = await fetch("/api/botschafter/email", {
              method: "POST",
  ...
  return (
    <>
      <Modal isOpen={isOpen} onClose={onClose} size={"xl"}>
        <ModalOverlay />
        <ModalContent>
          <ModalHeader>Botschafter Email austaschen</ModalHeader>
          <ModalCloseButton />
          <ModalBody>
            <Text mb={4}>Bitte CSV Datei auswählen:</Text>
            <Input
              ref={fileInput}
              name="emailBotschafterFile"
              type="file"
              sx={{
...
export default EmailBotschafterModal;
